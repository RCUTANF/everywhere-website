...
```bash
sudo apt update && \
sudo apt install -y fontconfig libx11-6 libice6 libsm6 hicolor-icon-theme
```
...
```bash
sudo yum install -y fontconfig libX11 libICE libSM hicolor-icon-theme
```
...
```bash
sudo dnf install -y fontconfig libX11 libICE libSM hicolor-icon-theme
```
...
```bash
sudo pacman -Sy --noconfirm fontconfig libx11 libice libsm hicolor-icon-theme
```

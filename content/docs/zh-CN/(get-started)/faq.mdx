...
对于其它事实上主流的模型提供商（例如 LM Studio、通义千问 Qwen、智谱清言 GLM、豆包 Doubao 等），在兼容现有 API 协议的前提下，可以查阅<DynamicLink href="/[lang]/docs/model-provider/other">其他提供商</DynamicLink>在进阶模式进行手动配置和使用。
...
**Everywhere** 通过 API 与这些模型进行交互。因此，要启用网络搜索功能，需要借助一个外部的搜索引擎服务（例如 Google, Brave, Tavily, Bocha 等）。您需要在 **Everywhere** 中配置相应的搜索引擎服务，才能让 AI 在回答问题时具备联网搜索的能力。详见<DynamicLink href="/[lang]/docs/chat-plugin/web-browser">聊天插件 - 网络浏览器</DynamicLink>。
